...
> **Агент-специфичные инструкции:** Hermes → Aisystant MCP `get_instructions`.
...
**Новое (WP-XXX, 2026-06-04).** Project-scoped knowledge base для headless subagent'ов. НЕ для пользователя — read-only для человека. Структура:
- `domains/` — знание по Pack-доменам (personal-development, digital-platform, ...)
- `skills/` — capture из `.claude/skills/SKILL.md` выполнения
- `patterns/` — cross-domain heuristics (multi-agent coordination, drift signals)
- `state/` — ephemeral runtime (`.gitignored`)

**Когда писать:** субагент обнаружил паттерн → атомарно в domain file. **Когда читать:** субагент инициализируется (hydrate state). **Подробно:** `.claude/agent-memory/README.md` и `.claude/agent-memory/SPEC.md`.
...
> **WP-394 Ф4.2.** Ниже — единое ядро инструкций для всех агентов (Claude, Kimi, Hermes). Правки — сюда.
